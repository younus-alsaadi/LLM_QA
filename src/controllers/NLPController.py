...
class NLPController(BaseController):
    ...
    def index_into_vector_db(self,project: Project,chunks:List[DataChunk], chunks_ids: List[int],
        ...
        vectors = [
            self.embedding_model_client.embed_text(text=text,document_type=DocumentTypeEnum.DOCUMENT.value)
            for text in chunks_as_text
        ]
    ...
    def search_vector_db_collection(self, project: Project, text: str, limit: int = 10):
        ...
        vector = self.embedding_model_client.embed_text(text=text,
                                                 document_type=DocumentTypeEnum.QUERY.value)

        print(f"len vector {len(vector)}")  # it's 1536 right now
